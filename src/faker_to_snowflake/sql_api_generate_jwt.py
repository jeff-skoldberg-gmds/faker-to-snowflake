...
class JWTGenerator(object):
    ...
    def __init__(
        ...
        logger.info(
            f"Creating JWTGenerator with arguments\n"
            f"account : {account}, user : {user}, lifetime : {lifetime}, renewal_delay : {renewal_delay}"
        )
    ...
    def get_token(self) -> Text:
        ...
        if self.token is None or self.renew_time <= now:
            logger.info(
                f"Generating a new token because the present time ({now}) is later than the renewal time ({self.renew_time})"
            )
